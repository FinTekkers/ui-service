<script lang="ts">
  ...
  const displayError = (fieldName: string) => {
    if (form?.formError) {
      ...
      const errors: string[] =
        (form.formError as unknown as Record<string, string[]>)[fieldName] ||
        [];
...
<div class="sign_in_fields">
  ...
  <label for="password" class="passwordField">
    ...
    {#if $booleanStore.IS_PASSWORD_VISIBLE}
      <span
        class="togglePasswordDisplayIcon"
        on:keydown={() => "x"}
        on:click={() =>
          customBooleanStoreUpdater(booleanKeys.IS_PASSWORD_VISIBLE)}
      >
        <Icon icon="solar:eye-linear" style="width: 25px; height: 25px;" />
      </span>
    {:else}
      <span
        class="togglePasswordDisplayIcon"
        ...
        on:click={() =>
          customBooleanStoreUpdater(booleanKeys.IS_PASSWORD_VISIBLE)}
      >
        <Icon icon="clarity:eye-hide-line" style="width: 25px; height: 25px;" />
      </span>
    {/if}
    <div class="error_message">
